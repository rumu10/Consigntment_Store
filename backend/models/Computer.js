...
class Computer {
    ...
    const HumanReadableToGB = (sizeStr) => {
        ...
        if (fSize.includes('GB')) {
            ...
            console.log("Input for humanReadableToGB only supports GB/TB input. Current input is", sizeStr);
        }
        return s;
    }
    ...
    const GBToHumanReadable = (size) => {
        ...
        if (s >= 1024) {
            console.log("Input for GBToHumanReadable only supports GB/TB input. Current input is in GB: ", size);
        }
        return sizeStr;
    }
